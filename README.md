...
> ✅ **Real RAG, honestly scoped.** Answers are generated by **Google Gemini** (free tier)
> from a fixed 4-document corpus, with **citations rebuilt server-side** so every claim maps to
> an exact page. Still simulated for now (and clearly marked below): **vector retrieval** (today
> the whole corpus is sent to the model), **token streaming**, and **document uploads** - see
...
and **rebuilds every citation from the corpus server-side**, so a snippet is always an exact
substring of its page (exactly what the drawer highlights) - the model never authors the
snippet. The `GEMINI_API_KEY` lives only on the server. If the API is unavailable the UI
falls back to the sample answers, so the demo never breaks.

| Done | On the roadmap |
| --- | --- |
| `send()` → `fetch('/api/chat')` with graceful fallback | Real **token streaming** (SSE / `ReadableStream`) replacing the simulated reveal |
| **Generation** - Gemini, grounded, concise, `[[n]]` citations | **Vector retrieval** - embed the corpus → top-k per question (today the whole corpus is sent) |
| **Citations** rebuilt from the corpus (`docId`, `page`, exact snippet) | **Uploads** - parse → chunk → embed → vector store (pgvector) for user documents |
